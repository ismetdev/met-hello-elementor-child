# PROJECT LOG ARCHIVE: 2026, v1.0.0 to v1.8.0

Archived entries from [../PROJECT_LOG.md](../PROJECT_LOG.md). Newest first,
same as the live log. Nothing here has been edited, only moved.

**Covers:** the theme from its first scaffold (v1.0.0) through the sitewide
design token layer and the Elementor `<main>` landmark fix (v1.8.0), which is
2026-07-07 to 2026-08-04. The active log continues from v1.9.0, the block
system migration.

**Archived on 2026-08-07**, when the live log reached 712 lines. See
[DECISIONS D23](../DECISIONS.md#d23) for the rule and why it had to change:
every entry in this file is from the current year, so the original
"archive entries older than the current year" trigger could never fire.

**Provenance, carried over from the live log.** Reconstructed on 2026-08-01.
Versions 1.0.0 to 1.4.2 were built on the home machine, and Claude Code
transcripts do not sync between machines, so the office laptop has none of
those sessions. Those entries come from the `readme.txt` changelog, git
history, the code itself, and one cross-reference in the MetCPT session of
2026-07-29. Dates for 1.0.0 to 1.3.0 are phase attributions, not commit dates:
all three commits in this repo were made on 2026-07-07, when the finished
theme was first pushed to GitHub.
